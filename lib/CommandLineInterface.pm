...
sub addRequiredRelationship
  {
    ...
    #Make sure the file type IDs are unsigned integers
    if($file_type1 !~ /^\d+$/)
      {
    ...
    if($file_type2 >= scalar(@$input_files_array) ||
	...
	if(!$ok)
	  {
	    error("The option specification: [$get_opt_str] must end with ",
		  "'=s'.  Supply --debug to extend this error with an ",
		  "explanation.",
		  ($DEBUG ?
		   joing(',',
			 ("In order to shell-interpolate the values, ",
			  ...
			  "CommandLineInterface handles array options ",
			  "different from GetOpt::Long.  It adds mitigations ",
			  "of some shell-imposed command-line length ",
			  "limitations and can handle 2D arrays (or ",
			  "multiple 1D arrays of the same type).")) : ''));
	    $answer = 0;
	  }
      }
...
sub addOutfileSuffixOption
  {
    ...
    my $file_type_index =         #Val returned from addInfileOption
      (defined($in[1]) ? $in[1] : 0);  ##TODO: The default index definitely must be selected more intelligently. See requirement 179
	...
	while(my @caller_info = caller($stack_level))
	  {
	    ...
	    if(defined($calling_sub))
	      {$calling_sub =~ s/^.*?::(.+)$/$1/}
	    if($calling_sub eq 'addDefaultFileOptions')
	      {
		$called_implicitly = 1;
		last;
	      }
    ...
    if($file_type_index >= scalar(@$input_files_array))
      {
	error("Outfile suffix option [$get_opt_str] has been added to be ",
	      "take a suffix that is appended to input file type ",
	...
	while(my @caller_info = caller($stack_level))
	  {
	    ...
	    if(defined($calling_sub))
	      {$calling_sub =~ s/^.*?::(.+)$/$1/}
	    if($calling_sub eq 'getInfile' || $calling_sub eq 'getOutfile')
	      {
		$called_implicitly = 1;
		last;
	      }
...
sub addDefaultFileOptions
  {
    ...
    #If the programmer did not add an outfile suffix option and the programmer
    #added 1 or 0 outfile options, add a default
    #This is because the default should only be added if there's only 1 type of
    #output file, so that the default output file name can be obtained
    #unambiguously, as an alternative to supplying an outfile name (both the
    #file created by the suffix or named by --outfile are for the same output)
    if($num_suffix_types == 0 && $num_outfile_types <= 1)
      {addOutfileSuffixOption()}

    #If the programmer did not add an outfile option and the programmer added 1
    #or 0 outfile suffix options, add a default
    #This is because the default should only be added if there's only 1 type of
    #output file, so that the default output file name can be obtained
    #unambiguously, as an alternative to supplying an outfile suffix (both the
    #file created by the suffix or named by --outfile are for the same output)
    if($num_outfile_types == 0 && $num_suffix_types <= 1)
      {addOutfileOption()}
...
sub processCommandLine
  {
    ...
    #Look for default output file type conflicts.  If one of the outfile types
    #is a default addition and both types are supplied on the command line, the
    #default and the other outfile option are 2 files being specified for the
    #same output (because the programmer doesn't have an ID to obtain the
    #separate output file names).
    #If there are 2 outfile types, one is a default suffix/named outfile, and
    #one of each type was supplied on the command line
    if(scalar(@$suffix_id_lookup) == 2 &&
       ($default_outfile_suffix_added || $default_outfile_added) &&
       scalar(#The suffix is defined
	      grep {defined($_)}

	      #Convert to contained suffixes (i.e. the values of the suffixes)
	      map {@{$outfile_suffix_array->[$_]}}

	      #The associated infile array is populated (i.e. -i or --outfile
	      #because it's saved as an infile and marked in the
	      #outfile_types_hash)
	      grep {defined($input_files_array->[$_]) &&
		      scalar(@{$input_files_array->[$_]})}
	      ...
	      0..$#{$outfile_suffix_array}) == 2)
      {
	##TODO: Instead of only catching the case where there are 2 outfile
	##      types and 1 was default-added, enhance this to know when a
	##      default is added and supplied on the command line with any
	##      other outfile option.  See requirement 183.
...
sub requireFileRelationships
  {
	...
	debug({LEVEL => -1},"Checking relationship of file types ",
	      "[$test_ftype,$valid_ftype] is: [$relationship].");
	...
	my $valid_files = $input_files_array->[$valid_ftype];
	...
	#present. Like, if file type 1 is supplied, file type 2 can be supplied
	#with it in a specific relative number. Otherwise, file type 2 by
	#itself makes no sense. Note, it is assumed that global requirements
...
sub getNumFileGroups
  {
    ...
    return(scalar(@{$input_files_array->[$file_type_id]}));
  }
...
sub getStrSubParams
  {
    ...
    #Issue an error if any variable is encountered that is not a hash and not
    #a scalar value.
...
sub processDefaultOptions
  {
    ...
    verbose("Overwrite:      [$overwrite].")    if($overwrite);
    verbose('Skip Existing:  [on].')            if($skip_existing);
    verbose('Append:         [on].')            if($append);
    verbose("Dry run mode:   [$dry_run].")      if($dry_run);
    verbose("Collision mode: [$collide_mode].") if($collide_mode ne '');
...
sub flushStderrBuffer
  {
	...
	my($type,$level,$message,$leader,$smpl_msg,$smpl_ldr) =
	  @$message_array;
...
sub customHelp
  {
    ...
    if($default_outfile_added && $default_outfile_suffix_added)
      {
	my $usage_hash =
	  (grep {($_->{OPTTYPE} eq 'outfile' || $_->{OPTTYPE} eq 'suffix') &&
		   !$_->{HIDDEN}} @$usage_array)[0];
	my $usage_hash2 =
	  (grep {($_->{OPTTYPE} eq 'outfile' || $_->{OPTTYPE} eq 'suffix') &&
		   !$_->{HIDDEN}} @$usage_array)[1];
	...
	if(!defined($desc) || $desc eq '')
	  {
	    $desc = join('','The author of this script has not provided ',
			 'a format description for this input file ',
    ...
    #Else if one of the default outfile types was explicitly added and the
    #other added by default, then it's assumed there are only 2 outfile types.
    #Pick the one that was added explicitly.
	...
	my $flag = '* ' . "OUTPUT FORMAT:\n" . $usage_hash->{OPTFLAG} .
	  ',' . $usage_hash2->{OPTFLAG};
	my $desc = (defined($usage_hash->{FORMAT}) &&
		    $usage_hash->{FORMAT} ne '' ?
		    $usage_hash->{FORMAT} : $usage_hash2->{FORMAT});
...
sub getSummaryUsageOptStr
  {
    ...
    my $optionals          = ($local_extended ? '' : '...');
    my $optionals_wo_prim  = ($local_extended ? '' : '...');
    my $requireds          = '';
...
=item C<addInfileOption> GETOPTKEY [, REQUIRED, DEFAULT, PRIMARY, SMRY_DESC, DETAIL_DESC, FORMAT_DESC, PAIR_WITH, PAIR_RELAT]
...
To construct output file names with an immutable, always-present output file name (e.g. 'output.log') that the user should not see as an option in the usage or help output, set HIDDEN to a non-zero value.
...
To construct output file names with an immutable, always-present suffix (e.g. '.log') that the user should not see as an option in the usage or help output, set HIDDEN to a non-zero value.
